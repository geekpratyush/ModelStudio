...
function makeDraggable(el, handle) {
    ...
    handle.onmousedown = (e) => {
        ...
        const mm = (me) => { pos1 = pos3 - me.clientX; pos2 = pos4 - me.clientY; pos3 = me.clientX; pos4 = me.clientY; el.style.top = (el.offsetTop - pos2) + "px"; el.style.left = (el.offsetLeft - pos1) + "px"; };
...
function renderConnections() {
    ...
    if (!svg) return;
    svg.querySelectorAll('.wire-group').forEach(w => w.remove());
...
function pushContainer(containerId, dx, dy, visited = new Set()) {
    ...
    if (dx !== 0 || dy !== 0) {
        ...
        systemState.nodes.forEach(n => { if (n.parentContainerId === containerId) { if (n.isContainer) pushContainer(n.id, dx, dy, visited); else { n.x += dx; n.y += dy; } } });
    }
    const cw = container.width || 480, ch = container.height || 320, pad = 10;
    systemState.nodes.forEach(other => {
        if (other.isContainer && other.id !== containerId && !visited.has(other.id)) {
            ...
            if (overlapX && overlapY) {
                let pdx = 0, pdy = 0, activePush = false;
                if (dx !== 0 || dy !== 0) {
                    const movingRight = dx > 0 && container.x + cw > other.x && container.x < other.x, movingLeft = dx < 0 && container.x < other.x + ow && container.x + cw > other.x + ow;
                    const movingDown = dy > 0 && container.y + ch > other.y && container.y < other.y, movingUp = dy < 0 && container.y < other.y + oh && container.y + ch > other.y + oh;
                    if (movingRight || movingLeft) { pdx = dx; activePush = true; } if (movingDown || movingUp) { pdy = dy; activePush = true; }
                } else {
                    activePush = true; const midX = container.x + cw / 2, midY = container.y + ch / 2, omidX = other.x + ow / 2, omidY = other.y + oh / 2;
                    if (Math.abs(omidX - midX) > Math.abs(omidY - midY)) pdx = omidX > midX ? 10 : -10; else pdy = omidY > midY ? 10 : -10;
                }
                if (activePush) pushContainer(other.id, pdx, pdy, visited);
            }
        }
    });
}
...
function exitContext(nodeId) {
    ...
    if (p) {
        ...
        if (best.dir === 'L' || best.dir === 'R') n.x = best.val; else n.y = best.val;
...
function deleteSelectedObject(id, type) {
    if (type === 'node') { systemState.nodes = systemState.nodes.filter(n => n.id !== id); systemState.connections = systemState.connections.filter(c => c.from !== id && c.to !== id); AudioFX.play('trash'); }
...
function initApp() { 
    initLeftDrawerTabStripSelectors(); initCanvasEvents(); initPaletteEvents(); initThemeEngine(); initModals(); initPropertiesEditor(); initContextMenu(); initCustomPalette(); initIconPicker(); initInteractionToolbar(); initSketchToolbar();
    makeDraggable(document.getElementById('floating-prop-panel'), document.getElementById('floating-prop-handle')); 
    const sketchSvg = document.getElementById('svg-sketch-layer'); if (sketchSvg && typeof rough !== 'undefined') roughCanvas = rough.svg(sketchSvg);
    if (loadState()) { renderCustomPalette(); autoParentNodes(); enforcePhysics(); updateSketchToolbarVisibility(); }
    else { updateSketchToolbarVisibility(); }
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'v' || e.key === 'V') setInteractionMode('move'); if (e.key === 'h' || e.key === 'H') setInteractionMode('pan'); if (e.key === 'm' || e.key === 'M') setInteractionMode('select');
        if (e.key === 'p' || e.key === 'P') setInteractionMode('pencil'); if (e.key === 'r' || e.key === 'R') setInteractionMode('rect'); if (e.key === 'c' || e.key === 'C') setInteractionMode('circle');
        if (e.key === 'd' || e.key === 'D') setInteractionMode('diamond'); if (e.key === 'a' || e.key === 'A') setInteractionMode('arrow'); if (e.key === 'l' || e.key === 'L') setInteractionMode('dline');
        if (e.key === 't' || e.key === 'T') setInteractionMode('text'); if (e.key === 'e' || e.key === 'E') setInteractionMode('eraser');
    });
}
...
function renderSubsystemsTopologyCanvas(skipPanel = false) {
    ...
    systemState.nodes.forEach(n => {
        ...
        if (exitBtn) exitBtn.onclick = (e) => { e.stopPropagation(); exitContext(n.id); }; if (delBtn) delBtn.onclick = (e) => { e.stopPropagation(); deleteSelectedObject(n.id, 'node'); }; if (editBtn) editBtn.onclick = (e) => { e.stopPropagation(); openInspector(n.id, 'node'); };
        if (colorInput) colorInput.oninput = (e) => { n.color = e.target.value; renderSubsystemsTopologyCanvas(true); saveState(); };
        const resizer = el.querySelector('.resize-handle'); if (resizer) { resizer.onmousedown = (e) => { e.stopPropagation(); let sx = e.clientX, sy = e.clientY, sw = n.width || 480, sh = n.height || 320; const mm = (me) => { n.width = Math.max(200, sw + (me.clientX - sx) / systemState.zoom); n.height = Math.max(150, sh + (me.clientY - sy) / systemState.zoom); pushContainer(n.id, 0, 0, new Set([n.id])); renderSubsystemsTopologyCanvas(true); }; const mu = () => { window.removeEventListener('mousemove', mm); window.removeEventListener('mouseup', mu); saveState(); }; window.addEventListener('mousemove', mm); window.addEventListener('mouseup', mu); }; }
        el.querySelectorAll('.port').forEach(port => { port.onmousedown = (e) => { e.stopPropagation(); const portDir = port.className.split('port-')[1], startCoords = getPortCoords(n, portDir), svg = document.getElementById('svg-edge-layer'), canvasContainer = document.getElementById('canvas-container'); const tempPath = document.createElementNS("http://www.w3.org/2000/svg", "path"); tempPath.setAttribute('stroke', '#60a5fa'); tempPath.setAttribute('stroke-width', '2'); tempPath.setAttribute('fill', 'none'); tempPath.setAttribute('stroke-dasharray', '2,2'); svg.appendChild(tempPath); const mm = (me) => { const rect = canvasContainer.getBoundingClientRect(); const curX = (me.clientX - rect.left - systemState.panX) / systemState.zoom, curY = (me.clientY - rect.top - systemState.panY) / systemState.zoom; const d = `M ${startCoords.x} ${startCoords.y} C ${startCoords.x + (portDir === 'right' ? 50 : (portDir === 'left' ? -50 : 0))} ${startCoords.y + (portDir === 'bottom' ? 50 : (portDir === 'top' ? -50 : 0))} ${curX} ${curY} ${curX} ${curY}`; tempPath.setAttribute('d', d); }; const mu = (me) => { tempPath.remove(); window.removeEventListener('mousemove', mm); window.removeEventListener('mouseup', mu); const targetEl = document.elementFromPoint(me.clientX, me.clientY), portEl = targetEl ? targetEl.closest('.port') : null; if (portEl) { const targetNodeEl = portEl.closest('.node'); if (targetNodeEl && targetNodeEl.id !== n.id) { const targetPortDir = portEl.className.split('port-')[1]; const wire = { id: 'w_' + Date.now(), from: n.id, fromPort: portDir, to: targetNodeEl.id, toPort: targetPortDir, pattern: 'dotted', direction: 'forward', animated: true }; systemState.connections.push(wire); AudioFX.play('snap'); saveState(); renderSubsystemsTopologyCanvas(); } } }; window.addEventListener('mousemove', mm); window.addEventListener('mouseup', mu); }; });
        el.oncontextmenu = (e) => { e.preventDefault(); e.stopPropagation(); systemState.selectedIds = [n.id]; renderSubsystemsTopologyCanvas(); const menu = document.getElementById('context-menu'); if (menu) { document.getElementById('cm-anim-toggle').style.display = 'none'; const isIcon = n.type === 'icon-only'; document.getElementById('cm-convert').style.display = isIcon ? 'flex' : 'none'; document.getElementById('cm-edit').style.display = isIcon ? 'none' : 'flex'; menu.style.display = 'flex'; menu.style.left = e.clientX + 'px'; menu.style.top = e.clientY + 'px'; } };
        el.onmousedown = (e) => {
            ...
            e.stopPropagation(); const isMulti = e.shiftKey || e.ctrlKey || e.metaKey; if (!isMulti && !systemState.selectedIds.includes(n.id)) systemState.selectedIds = [n.id]; else if (isMulti) { if (systemState.selectedIds.includes(n.id)) systemState.selectedIds = systemState.selectedIds.filter(id => id !== n.id); else systemState.selectedIds.push(n.id); }
            ...
            const mm = (me) => {
                ...
                dragStates.forEach(s => { if (visited.has(s.node.id)) return; if (s.node.isContainer) { const curDx = (Math.round((s.ox + dx) / 10) * 10) - s.node.x, curDy = (Math.round((s.oy + dy) / 10) * 10) - s.node.y; if (curDx !== 0 || curDy !== 0) pushContainer(s.node.id, curDx, curDy, visited); } else { let nx = Math.round((s.ox + dx) / 10) * 10, ny = Math.round((s.oy + dy) / 10) * 10; autoParentNodes(); if (s.node.parentContainerId) { const p = systemState.nodes.find(node => node.id === s.node.parentContainerId); if (p) { const pad = 30, h_off = 25, cel = document.getElementById(s.node.id), nw = cel ? cel.offsetWidth : (n.width || 140), nh = cel ? cel.offsetHeight : (n.height || 65); let npx = p.x, npy = p.y, npw = p.width, nph = p.height, expanded = false; if (nx < p.x + pad) { const diff = (p.x + pad) - nx; npx -= diff; npw += diff; expanded = true; } if (ny < p.y + pad + h_off) { const diff = (p.y + pad + h_off) - minY; npy -= diff; nph += diff; expanded = true; } if (nx + nw > p.x + p.width - pad) { npw = (nx + nw - p.x) + pad; expanded = true; } if (ny + nh > p.y + p.height - pad) { nph = (ny + nh - p.y) + pad; expanded = true; } if (expanded) { p.x = npx; p.y = npy; p.width = npw; p.height = nph; pushContainer(p.id, 0, 0, new Set([p.id])); } nx = Math.max(p.x + pad, Math.min(nx, p.x + p.width - nw - pad)); ny = Math.max(p.y + pad + h_off, Math.min(ny, p.y + p.height - nh - pad)); } } s.node.x = nx; s.node.y = ny; enforcePhysics(); visited.add(s.node.id); } });
        ...
        el.className = `node ${n.isContainer ? 'node-actor-container' : ''} ${systemState.selectedIds.includes(n.id) ? 'selected' : ''} ${n.type === 'icon-only' ? 'icon-only-node' : ''}`;
        ...
        if (fc) {
            ...
            else { (n.fields || []).forEach(f => { const row = document.createElement('div'); row.className = 'node-field-row'; const typeDisp = f.type ? `<span style="opacity:0.6; font-size:0.65rem;">(${f.type})</span> ` : ''; row.innerHTML = `<i class="fa-solid fa-circle-info"></i><span class="node-field-name">${f.name}</span> ${typeDisp}: <span class="node-field-value">${f.value||'...'}</span>`; fc.appendChild(row); }); }
        }
    });
    requestAnimationFrame(() => { renderConnections(); renderSketches(); });
...
function initLeftDrawerTabStripSelectors() { document.querySelectorAll('.tab-btn').forEach(btn => { btn.onclick = () => { document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active')); document.querySelectorAll('.palette-viewport').forEach(p => p.classList.remove('active')); btn.classList.add('active'); const target = document.getElementById(btn.dataset.target); if (target) target.classList.add('active'); }; }); }
            ...
            if (systemState.interactionMode === 'pan' || e.button === 1) {
                ...
                const mu = (me) => { 
                    ...
                    systemState.nodes.forEach(n => {
                        ...
                        if (el) {
                            ...
                            if (nr.left <= r.right && nr.right >= r.left && nr.top <= r.bottom && nr.bottom >= r.top) {
                                systemState.selectedIds.push(n.id);
                            }
                        }
                    }); 
                    systemState.sketches.forEach(s => {
                        ...
                        const mX1 = (r.left - rect.left - systemState.panX) / systemState.zoom;
                        const mX2 = (r.right - rect.left - systemState.panX) / systemState.zoom;
                        const mY1 = (r.top - rect.top - systemState.panY) / systemState.zoom;
                        const mY2 = (r.bottom - rect.top - systemState.panY) / systemState.zoom;
                        
                        // Overlap check instead of fully-enclosed
                        const overlapX = (x1 <= mX2) && (x2 >= mX1);
                        const overlapY = (y1 <= mY2) && (y2 >= mY1);
                        if (overlapX && overlapY) systemState.selectedIds.push(s.id);
                    });
                ...
                let sketch = { 
                    id: 's_' + Date.now(), type: systemState.interactionMode, 
                    color: systemState.sketchSettings.color, strokeWidth: parseInt(systemState.sketchSettings.width),
                    style: systemState.sketchSettings.style, pattern: systemState.sketchSettings.pattern, 
                    animated: systemState.sketchSettings.animated
                };
                if (sketch.type === 'pencil') sketch.points = [{x: sx, y: sy}]; 
                else { sketch.x = sx; sketch.y = sy; sketch.w = 0; sketch.h = 0; sketch.x1 = sx; sketch.y1 = sy; sketch.x2 = sx; sketch.y2 = sy; }
                
                const mm = (me) => { 
                    const cx = (me.clientX - rect.left - systemState.panX) / systemState.zoom;
                    const cy = (me.clientY - rect.top - systemState.panY) / systemState.zoom; 
                    if (sketch.type === 'pencil') sketch.points.push({x: cx, y: cy}); 
                    else { sketch.w = cx - sx; sketch.h = cy - sy; sketch.x2 = cx; sketch.y2 = cy; } 
                    systemState.activeSketch = sketch; 
                    renderSubsystemsTopologyCanvas(true); 
                };
                
                const mu = () => { 
                    if (systemState.activeSketch) { 
                        systemState.sketches.push(systemState.activeSketch); 
...
function initPaletteEvents() { document.querySelectorAll('.palette-item').forEach(item => { item.ondragstart = () => { systemState.draggedItemData = { type: item.dataset.type, icon: item.dataset.icon, label: item.dataset.label, color: item.dataset.color, isContainer: item.dataset.type === 'actor-home', editorType: item.dataset.editorType }; }; }); }
function initThemeEngine() { const sel = document.getElementById('theme-selector'); if (sel) { sel.onchange = () => { document.body.className = sel.value === 'default' ? '' : `theme-${sel.value}`; systemState.activeTheme = sel.value; saveState(); }; } }
function initModals() {
    ...
    document.querySelectorAll('.help-nav-item').forEach(item => {
        item.onclick = () => {
            ...
            if (target) {
                document.querySelectorAll('.help-topic').forEach(topic => topic.classList.remove('active'));
    ...
    if (helpSearch) {
        helpSearch.oninput = (e) => {
            ...
            document.querySelectorAll('.help-topic').forEach(topic => {
                ...
                // If searching, show all matches and hide nav sidebar if preferred, 
                // or just highlight the nav items. Let's keep it simple: 
                // filter topics and show them if they match.
                if (term.length > 0) {
                    topic.style.display = match ? 'block' : 'none';
                    topic.classList.add('active'); // show all matching ones
                } else {
                    topic.style.display = ''; // reset to CSS defaults
                    // reset to the first one being active
                    ...
                    if (activeNav) {
                        const t = document.getElementById(activeNav.dataset.target);
    ...
    document.querySelectorAll('.btn-icon-small').forEach(btn => { 
        btn.onclick = () => Object.values(modals).forEach(m => m.classList.remove('open')); 
    });
    document.querySelectorAll('.modal-shade').forEach(m => { m.onclick = (e) => { if (e.target === m) m.classList.remove('open'); }; });
    document.getElementById('btn-apply-code').onclick = () => { try { const data = JSON.parse(document.getElementById('topology-code-area').value); systemState.nodes = data.nodes || []; systemState.connections = data.connections || []; systemState.sketches = data.sketches || []; autoParentNodes(); enforcePhysics(); renderSubsystemsTopologyCanvas(); modals.code.classList.remove('open'); saveState(); } catch(e) { alert("Invalid JSON Blueprint"); } };
    document.querySelectorAll('.template-card').forEach(card => { 
        card.onclick = () => { 
            ...
            if (t) { 
                try {
                    // deep clone to avoid modifying original template data
                    ...
                    enforcePhysics(); 
                    renderSubsystemsTopologyCanvas(); 
                    modals.templates.classList.remove('open'); 
                    saveState(); 
                } catch (err) {
                    console.error("Template Loading Error:", err);
                    alert(`CRITICAL ERROR: Failed to load template "${templateId}".\n\nReason: ${err.message}\n\nPlease report this bug with the template name.`);
                }
            } else {
                alert(`ERROR: Template data for "${templateId}" not found in system library.`);
            }
        }; 
    });
    document.getElementById('btn-clear-canvas').onclick = () => { if (confirm("Reset the entire canvas? This cannot be undone.")) { systemState.nodes = []; systemState.connections = []; systemState.sketches = []; renderSubsystemsTopologyCanvas(); saveState(); } };
    ...
    const toggleSketchBtn = document.getElementById('btn-toggle-sketch');
    if (toggleSketchBtn) {
        toggleSketchBtn.onclick = () => {
            systemState.sketchToolbarVisible = !systemState.sketchToolbarVisible;
...
function updatePropertiesPanel() {
    ...
    if (node) {
        ...
        node.fields.forEach((f, idx) => { const row = document.createElement('div'); row.className = 'field-editor-row'; row.style.display = 'flex'; row.style.gap = '4px'; row.style.marginBottom = '4px'; row.innerHTML = `<input type="text" class="prop-input" value="${f.name}" style="flex:1;"><input type="text" class="prop-input" value="${f.value}" style="flex:1.5;"><button class="btn-icon-small"><i class="fa-solid fa-trash"></i></button>`; const inputs = row.querySelectorAll('input'); inputs[0].onchange = (e) => { f.name = e.target.value; renderSubsystemsTopologyCanvas(); saveState(); }; inputs[1].onchange = (e) => { f.value = e.target.value; renderSubsystemsTopologyCanvas(); saveState(); }; row.querySelector('button').onclick = () => { node.fields.splice(idx, 1); updatePropertiesPanel(); renderSubsystemsTopologyCanvas(); saveState(); }; list.appendChild(row); });
    }
...
function initCustomPalette() {
    ...
    if (btn) { btn.onclick = () => { const name = document.getElementById('cust-name').value, type = document.getElementById('cust-type').value, icon = document.getElementById('cust-icon').value, subtype = document.getElementById('cust-subtype').value; if (!name) return; const item = document.createElement('div'); item.className = 'palette-item'; item.draggable = true; item.dataset.type = type; item.dataset.icon = icon || 'fa-cube'; item.dataset.label = name; item.dataset.editorType = subtype; item.dataset.color = '#3b82f6'; item.innerHTML = `${getIconHtml(item.dataset.icon)}<span>${name}</span>`; item.ondragstart = () => { systemState.draggedItemData = { ...item.dataset, isContainer: type === 'actor-home' }; }; document.getElementById('grid-custom-items-container').appendChild(item); }; }
}
function initIconPicker() {
    ...
    if (btn) { btn.onclick = () => { modal.classList.add('open'); renderIconGrid(''); }; }
    function renderIconGrid(filter) { if (!grid) return; grid.innerHTML = ''; Object.values(ICON_CATEGORIES).flat().forEach(icon => { if (!filter || icon.includes(filter)) { const item = document.createElement('div'); item.className = 'icon-picker-item'; item.style.padding = '10px'; item.style.cursor = 'pointer'; item.style.textAlign = 'center'; item.innerHTML = `<i class="${icon}" style="font-size: 1.5rem;"></i>`; item.onclick = () => { const node = systemState.nodes.find(n => systemState.selectedIds.includes(n.id)); if (node) { node.icon = icon; renderSubsystemsTopologyCanvas(); saveState(); updatePropertiesPanel(); } modal.classList.remove('open'); }; grid.appendChild(item); } }); }
    if (search) search.oninput = (e) => renderIconGrid(e.target.value);
}
...
function initSketchToolbar() {
    ['pencil', 'rect', 'circle', 'diamond', 'arrow', 'dline', 'text', 'eraser', 'move', 'pan'].forEach(t => {
        const btn = document.getElementById('tool-' + (['move','pan'].includes(t) ? 's-'+t : t));
        if (btn) { btn.onclick = () => { setInteractionMode(t); }; }
    });
    const sc = document.getElementById('sketch-prop-color'); if (sc) sc.oninput = (e) => { systemState.sketchSettings.color = e.target.value; saveState(); };
    const sw = document.getElementById('sketch-prop-width'); if (sw) sw.oninput = (e) => { systemState.sketchSettings.width = e.target.value; saveState(); };
    const ss = document.getElementById('sketch-prop-style'); if (ss) ss.onchange = (e) => { systemState.sketchSettings.style = e.target.value; saveState(); };
    const sp = document.getElementById('sketch-prop-pattern'); if (sp) sp.onchange = (e) => { systemState.sketchSettings.pattern = e.target.value; saveState(); };
    const sa = document.getElementById('sketch-prop-animated'); if (sa) sa.onchange = (e) => { systemState.sketchSettings.animated = e.target.checked; saveState(); };
}
